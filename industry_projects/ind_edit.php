...
<div class="page-content">
    ...
    <form action="/~D1285210/industry_projects/update.php" method="post">
        ...
        <label><h3>合作單位</h3><br><input type="text" name="partners" placeholder="請輸入合作單位" value="<?= htmlspecialchars($data['partners']) ?>" style="width:80%; padding:10px; font-size: 16px;" required></label><br><br>
        <label><h3>計劃資金</h3><br><input type="number" name="amount" placeholder="例：10000" value="<?= htmlspecialchars($data['amount']) ?>" style="width:80%; padding:10px; font-size: 16px;" required max="99999999"></label><br><br>
        <label><h3>計劃簽署日期</h3><br><input type="date" name="signed_date" value="<?= $data['signed_date'] ?>" style="width:80%; padding:10px; font-size: 16px;" required></label><br><br>
        <label><h3>成果摘要</h3><br><textarea name="outcome" placeholder="請輸入成果摘要" style="width:80%; padding:10px; font-size: 16px;" required><?= htmlspecialchars($data['outcome']) ?></textarea></label><br><br>
