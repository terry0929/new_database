...
if (
    ...
    $stmt = $conn->prepare("INSERT INTO industry_projects
        (result_id, title, author, partners, amount, signed_date, outcome, upload_date, remarks)
        VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssissss", $result_id, $title, $author, $partners, $amount, $signed_date, $outcome, $upload_date, $remarks);
    $stmt->execute();
